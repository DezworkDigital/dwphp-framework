 ...
 * @Last Modified time: 2018-01-12 07:34:04
 */
	...
	class html{
         ...
         */
		public function includeFile($filename='',$local=''){
			if($local!='start' && $local!='end'){$local='end';}
			$ext = pathinfo($filename, PATHINFO_EXTENSION);
			if($ext=='css'){
				$this->setFilesCss($GLOBALS['f']->fileVersion($filename),$local);
			}else if($ext=='js'){
				$this->setFilesJS($GLOBALS['f']->fileVersion($filename),$local);
			}
		}

		/* GETTRES AND SETTRES */
		public function setFilesCss($filesCss,$local=''){
			if($local!='start' && $local!='end'){$local='end';}
			$this->filesCss[$local][] = $filesCss;
			return $filesCss;
		}
		public function getFilesCss(){
			if(isset($this->filesCss['start']) && is_array($this->filesCss['start'])){ $this->filesCss['start'] = array_unique($this->filesCss['start']);}
			if(isset($this->filesCss['end']) && is_array($this->filesCss['end'])){ $this->filesCss['end'] = array_unique($this->filesCss['end']);}
			return $this->filesCss;
		}
		public function getTagFilesCSS($local){
			if($local!='start' && $local!='end'){$local='end';}
			$files= $this->getFilesCss();
			$ret = '';
			if(isset($files[$local])){
				for ($i=0; $i < count($files[$local]); $i++) {
					if(isset($files[$local][$i])){
							$ret .= '<link href="'.$files[$local][$i].'" rel="stylesheet" type="text/css" />'."\n";
					}
				}
			}
			return $ret;
		...
		// FilesJS
		public function setFilesJS($filesJS,$local=''){
			if($local!='start' && $local!='end'){$local='end';}
			$this->filesJS[$local][] = $filesJS;
			return $filesJS;
		}
		public function getFilesJS(){
			if(isset($this->filesJS['start']) && is_array($this->filesJS['start'])){ $this->filesJS['start'] = array_unique($this->filesJS['start']);}
			if(isset($this->filesJS['end']) && is_array($this->filesJS['end'])){ $this->filesJS['end'] = array_unique($this->filesJS['end']);}
			return $this->filesJS;
		}

		public function getTagFilesJS($local){
			if($local!='start' && $local!='end'){$local='end';}
			...
			if(isset($files[$local])){
				for ($i=0; $i < count($files[$local]); $i++) {
					if(isset($files[$local][$i])){
							$ret .= '<script src="'.$files[$local][$i].'" type="text/javascript"></script>'."\n";
					}
				}
			}
			return $ret;
