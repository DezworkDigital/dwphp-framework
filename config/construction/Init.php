 ...
 * @Last Modified time: 2018-01-11 22:11:21
 */
	...
	public function getImportCurrentDataApplication(){
		...
		foreach ($app_config[$this->getApplicationName()] as $key => $value) {
            ...
            if(isset($value['address_uri'])){
                if( strpos($_SERVER['HTTP_HOST'], $addresUri[0]) !== false ){
...
/*
	...
	private function getApplicationPathFiles(){
		...
		if($helpers==true && file_exists($directory_action) && is_file($directory_action) ){
			...
			//caso não existir a view, inicia erro 404
	...
	public  function fileVersion($localFile=''){
		...
		unset($p[1]);
		$p=implode('/',$p);
		if(file_exists(PATH_ROOT.$this->getPublicPath().$localFile) && $localFile!=''){
		    return substr($this->getPathBaseHref(),0,-1).($p=='/default'?$p:'').$localFile.'?'.md5(filemtime(PATH_ROOT.$this->getPublicPath().$localFile)); exit;
		}else{
		    return $localFile;
		}
	}
